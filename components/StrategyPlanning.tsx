...
//contunue chatting functionality
  ...
  const [finalizedStrategies, setFinalizedStrategies] = useState<string[]>(() => {
    const savedStrategies = localStorage.getItem('finalizedStrategies');
    return savedStrategies ? JSON.parse(savedStrategies) : [];
  });

  //finalize strategy function
  const handleFinalizeStrategyClick = () => {
    if (request.length > 0) {
      setFinalizedStrategies((prevStrategies) => [...prevStrategies, ...request]);
      localStorage.setItem('finalizedStrategies', JSON.stringify([...finalizedStrategies, ...request]));
      setShowContent(true);
